...
本项目为 OpenHarmony 平台编译了 coreutils，并发布预构建包。

## 获取预构建包

前往 [release 页面](https://github.com/Harmonybrew/ohos-coreutils/releases) 获取。
...
注意：coreutils 和系统里面的 toybox 有很多命令是重合的，为了让 coreutils 的优先级更高，配置 PATH 的时候要把它排前面。
...
需要用一台 Linux x64 服务器来运行项目里的 build.sh，以实现 coreutils 的交叉编译。
